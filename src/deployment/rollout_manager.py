...
class RolloutManager:
    ...
    def get_phase_report(self) -> Dict:
        ...
        # Handle PAUSED and ROLLBACK states (no config for these)
        if self.current_phase in (RolloutPhase.PAUSED, RolloutPhase.ROLLBACK):
            ...
            # Calculate return based on initial value (peak value before drawdown)
            initial_value = metrics.peak_value if metrics.peak_value > 0 else 100
            return_pct = (metrics.current_value / initial_value - 1) * 100 if initial_value > 0 else 0
            return {
